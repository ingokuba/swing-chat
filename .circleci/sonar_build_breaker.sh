...
HTTP_STATUS=$(curl -s -o /dev/null -w '%{http_code}' -u $SONAR_TOKEN: $SONAR_SERVER/api/ce/task\?id\=$CE_TASK_ID)
...
ANALYSIS_ID=$(curl -XGET -s -u $SONAR_TOKEN: $SONAR_SERVER/api/ce/task\?id\=$CE_TASK_ID | jq -r .task.analysisId)
I=1
TIMEOUT=0
...
STATUS=$(curl -XGET -s -u $SONAR_TOKEN: $SONAR_SERVER/api/qualitygates/project_status?analysisId=$ANALYSIS_ID | jq -r .projectStatus.status)
